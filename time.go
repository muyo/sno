...
const (
	// Custom epoch as an offset to the Unix epoch, in seconds.
	...
	// The time unit timestamps are stored with - 4msec, handled as nanoseconds internally.
	TimeUnit = 4e6

	// Max number of time units that can be stored in an ID.
	// Corresponds to 2067-09-25 19:01:42.548 UTC in our custom epoch.
	MaxTimestamp = 1<<39 - 1

	// Max sequence supported by generators. Note that bounds can be set individually - this
	// is the upper cap.
	MaxSequence = 1<<16 - 1
